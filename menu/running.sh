...
Exp=$(curl -sS https://raw.githubusercontent.com/scpremiuman/Ijin-ip/main/ip | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
sls=$(systemctl status server-sldns | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
# DNS PATCH
#tipeos2=$(uname -m)
Name2=$(curl -sS https://raw.githubusercontent.com/scpremiuman/Ijin-ip/main/ip | grep $MYIP | awk '{print $2}')
# GETTING DOMAIN NAME
